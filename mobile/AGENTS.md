...
**Scaffold status:** Flutter project is in place (`dco_mobile`, iOS + Android). Theme, auth (mock in debug), four-tab shell, local Drift persistence (vehicles + outbox), register/edit vehicle, maintenance plan + history, parts, fuel logs, and expenses are implemented. Next: documents vault, then sync engine drain.
